...
float Plane::DistanceToPoint(const WorldPosition& point) const
{
...
bool ViewFrustum::IsAABBInFrustum(const AABB& aabb, const WorldPosition& position) const
{
    bool result = true;
    for (auto& plane : m_Planes)
    {
        if (plane.DistanceToPoint(aabb.getVP(plane.normal, position)) < 0)
            return false;
        else if (plane.DistanceToPoint(aabb.getVN(plane.normal, position)) < 0)
            result = true;
    }
    return result;
}
